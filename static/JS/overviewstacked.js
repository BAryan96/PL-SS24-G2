      ...
      function setupContextMenu(chartInstance, series) {
        ...
        chartInstance.getZr().on('contextmenu', function (params) {
          ...
          if (chartInstance.containPixel('grid', pointInPixel)) {
            ...
            const xIndex = chartInstance.convertFromPixel({ seriesIndex: 0 }, [params.offsetX])[0];
            ...
            colorPicker.onchange = function () {
              if (dataItem) {
                ...
                series[0].data[dataIndex] = dataItem;  // Ensure the data item is updated in the series array
...
async function addChartContainer() {
  ...
  newChartContainer.style.position = "relative"; // Ensure the container is positioned relative
  ...
  closeButton.style.background = "grey"; // Changed background to grey
  closeButton.style.color = "white";
  closeButton.style.border = "none";
  closeButton.style.borderRadius = "50%";
  closeButton.style.width = "20px"; // Adjusted size for better fit
  closeButton.style.height = "20px"; // Adjusted size for better fit
  closeButton.style.display = "flex";
  closeButton.style.alignItems = "center";
  closeButton.style.justifyContent = "center";
  closeButton.style.padding = "0"; // Ensure no extra padding
  closeButton.style.cursor = "pointer";
  closeButton.addEventListener("click", () => {
    newChartContainer.remove();
    chartCount--;
    updateChartContainerWidth(); // Update width when a chart is removed
  });
  ...
  columnSelectX.addEventListener("change", () => {
    if (["orderDate", "Launch"].includes(columnSelectX.value)) {
      ...
      // Move the date granularity select right below the column select
      selectContainer.insertBefore(dateGranularityLabelX, columnSelectX.nextSibling);
      selectContainer.insertBefore(dateGranularitySelectX, dateGranularityLabelX.nextSibling);
    } else {
  ...
  submitButton.addEventListener("click", async (event) => {
    ...
    if (xTable !== "none" && xColumn !== "none" && yTable !== "none" && yColumn !== "none") {
      if (["orderDate", "Launch"].includes(xColumn) && dateGranularitySelectX.value !== "none") {
      ...
      try {
        ...
        searchInput.addEventListener('input', () => {
          ...
          const options = filterContent.querySelectorAll('.filter-options label');
          options.forEach(option => {
        ...
        filterContent.addEventListener("change", () => {
          ...
          dataX.forEach((value, index) => {
            if ((checkedXValues.length === 0 || checkedXValues.includes(value)) &&
                (checkedYValues.length === 0 || checkedYValues.includes(dataY[index].toString()))) {
        ...
        // Setup context menu for color picking
  ...
  chartCount++;
  updateChartContainerWidth(); // Update width when a new chart is added
  resizeCharts(); // Resize charts when a new chart is added
}
