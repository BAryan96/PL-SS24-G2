...
$(document).ready(async function() {
    await loadChartsSequentially([
        ...
        { id: 'myChart6', tables: ['products'], columns: ['sum sales'], type: 'bar', aggregations: [''], filters: [] }, // Beispiel für Produkte und Verkaufszahlen
        { id: 'myChart7', tables: ['products'], columns: ['sum total'], type: 'pie', aggregations: [''], filters: [] } // Beispiel für Produkte und Verkaufszahlen
    ]);
});
...
async function fetchData(requestData) {
    return new Promise((resolve, reject) => {
        let url = "/getdata";
        if (requestData.table_name && requestData.column_name) {
            url = "/calculatestatsdetails";
        }
        $.ajax({
            url: url,
            type: "POST",
...
function createRequestData(config) {
    if (config.tables.length === 1 && config.columns.length === 1) {
        return {
...
async function initializeChart(config) {
    if (config.type === 'geo') {
        ...
        const requestData = createRequestData(config);
